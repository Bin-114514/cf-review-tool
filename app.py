...
def _fmt_avg_time(stats: dict[str, object]) -> str:
    """AC 为 0 时平均耗时无意义，显示 — 而非 0min"""
    if not stats["ac"]:
        return "—"
    return str(round(stats["avg_time"] / 60))  # type: ignore[operator]
...
def _render_weakness(handle: str) -> None:
    ...
    # analyze_tags 的 tags_map 传空 → 回退提交内联 problem.tags，省去逐场 standings 调用
