...
**定调：** 纯规则 + 统计引擎，零外部 API 依赖（不调 LLM）。

**三层递进：**
```
M2-1 (选择器) → M2-2 (启发式摘要) → M2-3 (弱点识别·多场交叉)
```
...
| 决策 | 结论 |
|------|------|
| 摘要/弱点识别引擎 | 纯启发式（规则+统计），不调 LLM API |
| 批量提交拉取 | 按需分页（`from` 参数递增），不暴力拉 `count=1000` |
| C2 — `build_timeline` 按题目分组 | 延后：M1-3 已完成 flat timeline，通过 `build_problem_timeline()` 追加分组视图 |
| AtCoder 支持 | 推迟到 M3 |
| 分享链接 | 推迟到 M3（需后端存储） |
